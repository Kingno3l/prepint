...
!(function (t, e) {
  typeof exports === 'object' && typeof module !== 'undefined'
    ...
    : typeof define === 'function' && define.amd
      ...
      : ((t = typeof globalThis !== 'undefined' ? globalThis : t || self).bootstrap = e(t.Popper));
}(this, (t) => {
  function e(t) {
  ...
  const n = {
    ...
    remove(t, e) {
      if (!s.has(t)) return;
      const i = s.get(t);
      i.delete(e), i.size === 0 && s.delete(t);
    },
  };
  ...
  const c = (t) => (l(t)
    ...
    : typeof t === 'string' && t.length > 0
      ...
      : null);
  const h = (t) => {
    if (!l(t) || t.getClientRects().length === 0) return !1;
  ...
  const f = () => (window.jQuery && !document.body.hasAttribute('data-bs-no-jquery')
    ? window.jQuery
    : null);
  const m = [];
  ...
  const v = (t, e = [], i = t) => (typeof t === 'function' ? t(...e) : i);
  const y = (t, e, i = !0) => {
  ...
  class U extends W {
    ...
    static jQueryInterface(t) {
      return this.each(function () {
        ...
        if (typeof t === 'string') {
          if (void 0 === e[t] || t.startsWith('_') || t === 'constructor') throw new TypeError(`No method named "${t}"`);
          e[t](this);
        }
      });
    }
  ...
  b(J);
  const Z = '.bs.swipe';
  const tt = `touchstart${Z}`;
  const et = `touchmove${Z}`;
  const it = `touchend${Z}`;
  const st = `pointerdown${Z}`;
  const nt = `pointerup${Z}`;
  const ot = { endCallback: null, leftCallback: null, rightCallback: null };
  const rt = {
  ...
  const ft = `slid${lt}`;
  const mt = `keydown${lt}`;
  const pt = `mouseenter${lt}`;
  const bt = `mouseleave${lt}`;
  const vt = `dragstart${lt}`;
  const yt = `load${lt}${ct}`;
  const wt = `click${lt}${ct}`;
  const At = 'carousel';
  const Et = 'active';
  const Ct = '.active';
  const Tt = '.carousel-item';
  const kt = Ct + Tt;
  const $t = { ArrowLeft: _t, ArrowRight: ut };
  const St = {
    interval: 5e3,
    keyboard: !0,
    pause: 'hover',
  ...
  class Ot extends W {
    ...
    _slide(t, e = null) {
      ...
      const s = t === ht;
      const n = e || w(this._getItems(), i, s, this._config.wrap);
      if (n === i) return;
      const o = this._getItemIndex(n);
      const r = (e) => j.trigger(this._element, e, {
        relatedTarget: n,
      ...
      const c = s ? 'carousel-item-next' : 'carousel-item-prev';
      n.classList.add(c),
      g(n),
      i.classList.add(l),
      n.classList.add(l),
  ...
  j.on(document, wt, '[data-bs-slide], [data-bs-slide-to]', function (t) {
    ...
    return s
      ...
      : B.getDataAttribute(this, 'slide') === 'next'
  ...
  const Pt = `hide${It}`;
  const xt = `hidden${It}`;
  const Mt = `click${It}.data-api`;
  const jt = 'show';
  const Ft = 'collapse';
  const zt = 'collapsing';
  const Ht = `:scope .${Ft} .${Ft}`;
  const Bt = '[data-bs-toggle="collapse"]';
  const qt = { parent: null, toggle: !0 };
  const Wt = { parent: '(null|element)', toggle: 'boolean' };
  class Rt extends W {
    constructor(t, e) {
      ...
      for (const t of i) {
        ...
        const i = K.find(e).filter((t) => t === this._element);
  ...
  const Jt = `show${Vt}`;
  const Zt = `shown${Vt}`;
  const te = `click${Vt}${Qt}`;
  const ee = `keydown${Vt}${Qt}`;
  const ie = `keyup${Vt}${Qt}`;
  const se = 'show';
  const ne = '[data-bs-toggle="dropdown"]:not(.disabled):not(:disabled)';
  const oe = `${ne}.${se}`;
  const re = '.dropdown-menu';
  const ae = p() ? 'top-end' : 'top-start';
  const le = p() ? 'top-start' : 'top-end';
  const ce = p() ? 'bottom-end' : 'bottom-start';
  const he = p() ? 'bottom-start' : 'bottom-end';
  const de = p() ? 'left-start' : 'right-start';
  const ue = p() ? 'right-start' : 'left-start';
  const _e = {
    autoClose: !0,
  ...
  class fe extends W {
    constructor(t, e) {
      ...
      (this._menu = K.next(this._element, re)[0]
    ...
    _getPlacement() {
      ...
      const e = getComputedStyle(this._menu).getPropertyValue('--bs-position').trim()
        === 'end';
    ...
    static clearMenus(t) {
      ...
      for (const i of e) {
        ...
        const n = s.includes(e._menu);
        if (
          s.includes(e._element)
          || (e._config.autoClose === 'inside' && !n)
          || (e._config.autoClose === 'outside' && n)
        ) continue;
        if (
    ...
    static dataApiKeydownHandler(t) {
      ...
      const o = fe.getOrCreateInstance(n);
  ...
  const Ee = `focusin${Ae}`;
  const Ce = `keydown.tab${Ae}`;
  const Te = 'backward';
  const ke = { autofocus: !0, trapElement: null };
  const $e = { autofocus: 'boolean', trapElement: 'element' };
  class Se extends q {
    constructor(t) {
      super(),
      (this._config = this._getConfig(t)),
  ...
  const Oe = '.sticky-top';
  const Ie = 'padding-right';
  const De = 'margin-right';
  class Ne {
    constructor() {
  ...
  const Pe = '.bs.modal';
  const xe = `hide${Pe}`;
  const Me = `hidePrevented${Pe}`;
  const je = `hidden${Pe}`;
  const Fe = `show${Pe}`;
  const ze = `shown${Pe}`;
  const He = `resize${Pe}`;
  const Be = `click.dismiss${Pe}`;
  const qe = `mousedown.dismiss${Pe}`;
  const We = `keydown.dismiss${Pe}`;
  const Re = `click${Pe}.data-api`;
  const Ke = 'modal-open';
  const Ve = 'show';
  const Qe = 'modal-static';
  const Xe = { backdrop: !0, focus: !0, keyboard: !0 };
  const Ye = {
  ...
  const ei = 'showing';
  const ii = 'hiding';
  const si = '.offcanvas.show';
  const ni = `show${Ge}`;
  const oi = `shown${Ge}`;
  const ri = `hide${Ge}`;
  const ai = `hidePrevented${Ge}`;
  const li = `hidden${Ge}`;
  const ci = `resize${Ge}`;
  const hi = `click${Ge}${Je}`;
  const di = `keydown.dismiss${Ge}`;
  const ui = { backdrop: !0, keyboard: !0, scroll: !1 };
  const _i = {
    backdrop: '(boolean|string)',
    keyboard: 'boolean',
    scroll: 'boolean',
    ...
    _maybeSanitize(t) {
      return this._config.sanitize
        ? (function (t, e, i) {
          ...
          const n = [].concat(...s.body.querySelectorAll('*'));
          for (const t of n) {
            const i = t.nodeName.toLowerCase();
      ...
      for (const e of t) {
        if (e === 'click') {
          ...
          j.on(this._element, t, this._config.selector, (t) => {
            ...
            (e._activeTrigger[t.type === 'focusin' ? Li : Si] = !0), e._enter();
          }),
          j.on(this._element, i, this._config.selector, (t) => {
            const e = this._initializeOnDelegatedTarget(t);
            (e._activeTrigger[t.type === 'focusout' ? Li : Si] = e._element.contains(t.relatedTarget)),
            e._leave();
          });
        }
      }
  ...
  const xi = { ...Ni.DefaultType, content: '(null|string|element|function)' };
  class Mi extends Ni {
    static get Default() {
      return Pi;
    }
  ...
  const Fi = `activate${ji}`;
  const zi = `click${ji}`;
  const Hi = `load${ji}.data-api`;
  const Bi = 'active';
  const qi = '[href]';
  const Wi = '.nav-link';
  const Ri = `${Wi}, .nav-item > ${Wi}, .list-group-item`;
  const Ki = {
    offset: null,
  ...
  class Qi extends W {
    constructor(t, e) {
      ...
      (this._rootElement = getComputedStyle(this._element).overflowY === 'visible'
        ? null
        : this._element),
      ...
      this._config.smoothScroll
        ...
        j.on(this._config.target, zi, qi, (t) => {
          ...
          if (e) {
            ...
            const s = e.offsetTop - this._element.offsetTop;
    ...
    _observerCallback(t) {
      ...
      const s = (this._rootElement || document.documentElement).scrollTop;
      const n = s >= this._previousScrollData.parentScrollTop;
  ...
  const cs = 'fade';
  const hs = 'show';
  const ds = '.dropdown-toggle';
  const us = `:not(${ds})`;
  const _s = '[data-bs-toggle="tab"], [data-bs-toggle="pill"], [data-bs-toggle="list"]';
  const gs = `.nav-link${us}, .list-group-item${us}, [role="tab"]${us}, ${_s}`;
  const fs = `.${ls}[data-bs-toggle="tab"], .${ls}[data-bs-toggle="pill"], .${ls}[data-bs-toggle="list"]`;
  class ms extends W {
    constructor(t) {
      super(t),
    ...
    show() {
      ...
      const i = e ? j.trigger(e, Yi, { relatedTarget: t }) : null;
  ...
  b(ms);
  const ps = '.bs.toast';
  const bs = `mouseover${ps}`;
  const vs = `mouseout${ps}`;
  const ys = `focusin${ps}`;
  const ws = `focusout${ps}`;
  const As = `hide${ps}`;
  const Es = `hidden${ps}`;
  const Cs = `show${ps}`;
  const Ts = `shown${ps}`;
  ...
  const $s = 'show';
  const Ss = 'showing';
  const Ls = { animation: 'boolean', autohide: 'boolean', delay: 'number' };
  const Os = { animation: !0, autohide: !0, delay: 5e3 };
  class Is extends W {
    constructor(t, e) {
      super(t, e),
      (this._timeout = null),
